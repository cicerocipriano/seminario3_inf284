...
struct solution {
  ...
  inline void apply_change(const change c, const ld new_value) {
    ...
    this->_value = this->_std_deviation = sqrt(_variance);
    if (c.change_type > 0) {
      std::pair<ul, ul> firstx = c.computer_sequence[0];
      ul prev, after,
          first_val = this->_computers.at(firstx.first)
                          ._markets[firstx.second]
                          ._events_avg,
          first_exchange_id = this->_computers.at(firstx.first)
                                  ._markets[firstx.second]
                                  ._exchange_id,
          prev_exchange_id, after_exchange_id, last_exchange_id;
  ...
  inline ld partial_eval(const change &c) const {
    ul sum_changed = 0, sum_remv = 0, next = 0, new_val, old_val, sc, sumx2,
       new_sum, new_var, move_val, remove_comp, add_comp;
    if (c.change_type > 0) {
      ;
      ...
      sumx2 = this->_variance + (_computers.size() * (pow(_mean, 2))),
  ...
  inline bool check_validity(std::vector<ul> &hard_constraints) const {
    for (std::pair<ul, computer> i : this->_computers) {
      if (!i.second.check_computer(hard_constraints)) {
        printf(
            "Invalid solution: computer %lu has more markets than allowed.\n",
            i.first);
        return false;
      }
    }
